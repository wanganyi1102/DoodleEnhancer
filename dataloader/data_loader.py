...
class CreateDataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.opt.isTrain:
            ...
            img_source, scale = paired_transform(self.opt, img_source)
            img_source = self.transform_augment(img_source)

            img_target, scale = paired_transform(self.opt, img_target)
...
def paired_transform(opt, image):
    scale_rate = 1.0
